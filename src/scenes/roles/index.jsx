...
const Users = () => {
    ...
    return <Box m="20px">
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={0}>
            <Header title="Роли" subtitle="Доступные для назначения пользователям роли" />
          <Button
              ...
              <Table>
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, role, col } = row;
                    const selectedUser = selected.indexOf(role) !== -1;

                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        <TableCell padding="checkbox">
                          <Checkbox checked={selectedUser} onChange={(event) => handleClick(event, role)} />
                        </TableCell>
